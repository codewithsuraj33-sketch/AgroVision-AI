    ...
    def _fallback_admin_audit_context():
        try:
            ...
            for product in products:
                try:
                    tags = json.loads(getattr(product, "tags_json", "") or "[]")
                except (TypeError, ValueError):
                    tags = []

                clean_tags = [tag for tag in tags if str(tag).strip()]
                if len(clean_tags) < 2:
                    weak_tag_product_count += 1

            recommendation_review = []
            for mapping in mappings[:8]:
                product = getattr(mapping, "product", None)
                recommendation_review.append(
                    {
                        "disease": getattr(mapping, "disease_label", "") or getattr(mapping, "disease_key", "Unknown"),
                        "crop": getattr(product, "category", "General"),
                        "product_name": getattr(product, "name", "Unassigned"),
                        "status": "Mapped" if product is not None else "Missing product",
                    }
                )
